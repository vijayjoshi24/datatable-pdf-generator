...
/**
 * 
 * @author Vijay Joshi This is the core class to render PDF content for data
 *         table format content. It utilizes Apache PDF utility and generalizes
 *         PDF content generation for data table specific use cases.
 */
public class PDFUtil {
	...
	public static void drawline(PDPageContentStream contentStream, float width, float startX, float startY)
			throws Exception {
	...
	public static void addTextOption(PDPageContentStream contentStream, float startX, float startY, String text,
	...
	public static void addText(PDPageContentStream contentStream, float startX, float startY, String text,
	 ...
	 */
	public static void addBoldTextOption(PDPageContentStream contentStream, float startX, float startY, String text,
			boolean option) throws Exception {
		if (option) {
			if (text != null) {
	...
	public static float addParagraph(PDPageContentStream contentStream, float width, float sx, float sy, String text,
			float fontSize, PDFont font, boolean justify) throws IOException {
	...
	public static void addBoldText(PDPageContentStream contentStream, float startX, float startY, String text,
			float fontSize) throws Exception {
		if (text != null) {
			text = text.replaceAll("\\P{Print}", "");
	...
	public static float addSplitText(PDPageContentStream contentStream, float startX, float startY, String text,
			float fontSize, int wrapTextLen) throws Exception {
		String[] wrT = null;
		String s = null;
	...
	/**
	 * headerList - List of MAP Items title - Header Title. X Axis position -
	 ...
	 * @throws Exception
	 */
	public static float generateDataTable(PDDocument document, PDPageContentStream contentStream, PDFInput pdfInput)
			throws Exception {
		...
		for (int i = 0; i < pdfInput.getPdfDataTable().getHeaderList().size(); i++) {
			...
			addBoldText(contentStream, Float.valueOf(header.get("xaxis")), startingY, header.get("title"),
					pdfInput.getPdfDataTable().getDtHeaderFntSize());
		}
		...
		for (int j = 0; j < pdfInput.getPdfDataTable().getDataSet().size(); j++) {
			...
			if (line == 0) {
				for (int k = 0; k < pdfInput.getPdfDataTable().getDataSet().get(j).size(); k++) {
					addText(contentStream, datasetXAxis.get(k), startingY,
							pdfInput.getPdfDataTable().getDataSet().get(j).get(k),
							pdfInput.getPdfDataTable().getDtDataFntSize());
				}
			} else if (line % 55 == 0) {
				contentStream.close();
